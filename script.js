/* AKSARA CHAT SYSTEM - FINAL PROTECTED VERSION */
...
// ==================== INTEGRASI OFFICIAL CHANNEL (UPDATED) ====================

function goToOfficialChannel() {
    // 1. Simpan Username
    const currentUser = myName || document.getElementById('username').value;
    if (currentUser) {
        localStorage.setItem('aksara_redirect_user', currentUser);
    }
    
    // 2. Simpan Room Asal (Untuk fitur "Kembali")
    // Ambil room dari variabel myRoom (jika sudah login) atau dari input box
    let currentRoomName = "";
    if (myRoom) {
        currentRoomName = myRoom.replace('aksara-v29/', '');
    } else {
        currentRoomName = document.getElementById('room').value.trim();
    }

    if (currentRoomName && currentRoomName.toLowerCase() !== 'aksara') {
        localStorage.setItem('aksara_origin_room', currentRoomName);
    }
...
window.onload = function() {
    // 1. Cek apakah ini user yang "Pulang" dari Official Channel?
    ...
    if (returnUser && returnRoom) {
        ...
        // Bersihkan flag agar tidak stuck
        localStorage.removeItem('aksara_origin_room');
        
        // Auto Start Chat (Seamless Entry)
        console.log("🔄 Auto-joining origin room...");
        setTimeout(() => {
            startChat();
        }, 500);
    } else {
        // Load normal history if exists
...
// ==================== CHAT SYSTEM (UPDATED PROTECTION) ====================
...
function startChat() {
    ...
    // === PROTEKSI ROOM AKSARA ===
    if (room === 'aksara') {
        showToast("Room 'Aksara' telah digunakan.", "error");
        // Optional: Bersihkan input room agar user sadar
    ...
    localStorage.setItem('aksara_room', roomRaw); // Simpan format asli (huruf besar/kecil)
